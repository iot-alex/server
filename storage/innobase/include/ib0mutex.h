...
struct TTASMutex {
	...
	{
		int32 oldval = MUTEX_STATE_UNLOCKED;
		return m_lock_word.compare_exchange_strong(oldval,
						MUTEX_STATE_LOCKED,
	...
	void exit() UNIV_NOTHROW
	{
		ut_ad(m_lock_word.load(std::memory_order_relaxed)
		      == MUTEX_STATE_LOCKED);
		m_lock_word.store(MUTEX_STATE_UNLOCKED, std::memory_order_release);
	}
		...
		UNIV_NOTHROW
	{
		...
		while (!try_lock()) {
			if (n_spins++ == max_spins) {
				...
				m_lock_word.compare_exchange_strong(oldval,
						MUTEX_STATE_WAITERS,
						std::memory_order_relaxed,
						std::memory_order_relaxed);
